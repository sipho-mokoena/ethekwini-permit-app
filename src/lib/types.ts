...
export interface Backend {
  auth: {
    requestPhoneOTP(phone: string): Promise<{ ok: boolean; cooldownUntil?: number }>
    verifyOTP(phone: string, code: string): Promise<{ user: User; sessionToken?: string }>
    createEmailSession(email: string, password: string): Promise<{ user: User }>
  ...
  db: {
    ...
    createUploadedDocument(doc: UploadedDocumentInput): Promise<UploadedDocument>
