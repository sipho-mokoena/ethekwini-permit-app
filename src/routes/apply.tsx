...
import { useCreateApplication, useCreateUploadedDocument } from '../hooks/useApplications'
...
const REQUIRED_DOCUMENTS = [
  { type: 'id_document', label: 'South African ID Document', description: 'Clear copy of your ID book or card' },
  { type: 'proof_of_residence', label: 'Proof of Residence', description: 'Municipal account or lease agreement' },
  { type: 'business_plan', label: 'Business Plan', description: 'Simple business plan or description' },
  { type: 'site_plan', label: 'Site Plan', description: 'Layout of your spaza shop location' },
]
...
function ApplyPage() {
  ...
  const [uploadedFiles, setUploadedFiles] = useState<Record<string, { fileId: string; filename: string; documentType: string }[]>>({})
  ...
  const handleFilesUploaded = (documentType: string, files: { fileId: string; filename: string; documentType: string }[]) => {
  ...
  const canProceedFromDocuments = REQUIRED_DOCUMENTS.every(doc => 
  ...
  const renderChecklist = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Make sure you have all these documents ready before starting your application
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="mt-6">
          <Button onClick={() => setCurrentStep('documents')} className="w-full">
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          <div><strong>Trade Name:</strong> {formData.tradeName}</div>
          <div><strong>Location:</strong> {formData.location}</div>
          <div><strong>Phone:</strong> {user?.phone}</div>
          {formData.businessType && <div><strong>Business Type:</strong> {formData.businessType}</div>}
          {formData.employeeCount && <div><strong>Employees:</strong> {formData.employeeCount}</div>}
          {formData.operatingHours && <div><strong>Hours:</strong> {formData.operatingHours}</div>}
