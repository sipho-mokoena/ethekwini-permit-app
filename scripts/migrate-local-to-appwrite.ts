...
async function migrateLocalToAppwrite() {
  try {
    ...
    if (!endpoint || !projectId) {
      throw new Error('Please set VITE_APPWRITE_ENDPOINT and VITE_APPWRITE_PROJECT_ID environment variables')
    }
    
    if (!adminEmail || !adminPassword) {
      throw new Error('Please set ADMIN_EMAIL and ADMIN_PASSWORD environment variables for authentication')
    }
    ...
    const applications = await localDb.getAll('applications') as LocalApplication[]
    const uploadedDocuments = await localDb.getAll('uploaded_documents') as LocalUploadedDocument[]
    const files = await localDb.getAll('files') as LocalFile[]
    
    console.log(`📊 Found ${users.length} users, ${applications.length} applications, ${uploadedDocuments.length} documents, ${files.length} files`)
    ...
    for (const app of applications) {
      try {
        ...
        console.error(`  ❌ Failed to migrate application ${app.tradeName}:`, error)
      }
    }
    ...
    for (const doc of uploadedDocuments) {
      try {
        ...
        if (!newAppId || !newFileId) {
          console.warn(`  ⚠️  Skipping document ${doc.filename} - missing application or file mapping`)
          continue
        }
    ...
    console.log(`- ${Object.keys(applicationIdMapping).length} applications migrated`)
